...
def draw_room(img, x, y, w, h, filled=True):
    ...
    if shape_type == 'rect':
        ...
        pts = np.array([
            [x, y],
            [x + w, y + random.randint(-5, 5)],
            [x + w + random.randint(-5, 5), y + h],
            [x + random.randint(-5, 5), y + h + random.randint(-5, 5)]
        ])
        cv.fillPoly(img, [pts], 255)

    # Obstacles
    for _ in range(random.randint(3, 7)):
        ox = x + random.randint(5, max(5, w - 20))
        oy = y + random.randint(5, max(5, h - 20))
        ow = random.randint(5, min(15, w - (ox - x)))
        oh = random.randint(5, min(15, h - (oy - y)))
...
def connect_rooms_with_corridor(img, room1, room2):
    ...
    thickness = random.randint(12, 20)
    ...
    for _ in range(random.randint(4, 7)):
        ...
        ow = random.randint(5, 10)
        oh = random.randint(5, 10)
        cv.rectangle(img, (tx, ty), (tx + ow, ty + oh), 0, -1)


def generate_overlapping_submaps(global_size=(1000, 1000), submap_size=(500, 500),
                                 overlap_ratio=0.5, num_rooms=6, seed=None,
    ...
    for _ in range(num_rooms):
        rw = random.randint(80, 130)
        rh = random.randint(80, 130)
    ...
    y2 = y1 + sub_h - overlap_px_h + random.randint(-10, 10)
    x2 = x1 + sub_w - overlap_px_w + random.randint(-10, 10)
    ...
    if apply_rotation:
        angle = random.uniform(-max_rotation, max_rotation)  # small angle rotation
